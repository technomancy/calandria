...
-- @param axis Axis ("x", "y", or "z")
-- @param height Pyramid height.
-- @param diameter Pyramid's base diameter
-- @param node_name Name of node to make pyramid of.
-- @return The number of nodes added.
function worldedit.cone(pos, axis, height, radius, node_name)
	local pointer = {
		x = pos.x,
		y = pos.y,
		z = pos.z,
	...
	local count = 0
	local relativeheight = math.max(height, -height)

	for steps = math.min(height,0),math.max(height,0) do
		pointer.y = pos.y+steps
		local stageradius = math.floor(radius * ((relativeheight - steps) / height))
		count = count + worldedit.cylinder(pointer, axis, 1, stageradius, node_name, false)
	end
