...
minetest.register_chatcommand("/cone", {
	params = "x/y/z/? <height> <diameter> <node>",
	description = "Add cone centered at WorldEdit position 1 along the x/y/z/? axis with height <height>, base diameter of <diameter>, composed of <nodes>",
	func = safe_region(function(name, param)
		local found, _, axis, height, radius, nodename = param:find("^([xyz%?])%s+([+-]?%d+)%s+(%d+)%s+(.+)$")
		height = tonumber(height)
		...
		local node = get_node(name, nodename)
		local count = worldedit.cone(worldedit.pos1[name], axis, height, radius, node)
		worldedit.player_notify(name, count .. " nodes added")
	end,
	function(name, param)
		if worldedit.pos1[name] == nil then
		...
		radius = tonumber(radius)
		height = tonumber(height)
		return math.ceil((radius ^ 2 * math.pi) * height / 3)
	end),
})
